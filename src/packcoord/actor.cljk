(ns packcoord.actor
  "PackCoordActor — the ISCO-08 8183 packing, bottling and labelling
  machine operators plant scheduling/logistics coordination actor as a
  `langgraph.graph/state-graph` (ADR-2607121000 / CLAUDE.md Actors
  section). One graph run = one plant-coordination operation request
  ...
  cloud-itonami-isco-8143's papercoord.actor.
  ...
  The unconditional invariant: the Packing, Bottling and Labelling
  Plant Scheduling Coordination Advisor can never directly commit an
  operating record the PackCoordGovernor refuses, and can never make
  this actor finalize a line-operation-execution decision (e.g.
  deciding to proceed with a specific packing/bottling/labelling run)
  or a plant-safety-clearance decision (e.g. declaring the plant
  safety cleared), and can never override a plant safety officer's
  judgment — every commit-record! call is gated behind `:decide`, and
  that decision class is a permanent hard block regardless of
  disposition path. This actor coordinates PLANT SCHEDULING/LOGISTICS
  ONLY — it never operates packaging line equipment itself, and it
  never makes a plant-safety-clearance decision itself."
...
(defn build-graph
  "Build a compiled PackCoordActor graph. `store` implements
  `packcoord.store/Store`. `advisor` implements
  `packcoord.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:packer-id (:packer-id request)
                                    :op (:op proposal)
